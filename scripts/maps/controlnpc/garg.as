...
const float MELEE_DAMAGE			= 50;

const float CD_STOMP					= 1.0;
const float STOMP_SPEED				= 250.0;

const float KICK_RANGE					= 90.0;
const float KICK_DAMAGE				= 50;

const float RANGE_DAMAGE			= 100; //stomp
...
class weapon_garg : CBaseDriveWeapon
{
	...
	void MeleeAttack( float flRange, int iDamage, int iDmgType )
	{
		...
		CBaseEntity@ pHurt = GargantuaCheckTraceHullAttack( flRange + 10.0, iDamage, iDmgType );
		if( pHurt !is null )
		{
	...
	void StompAttack()
	{
		...
		g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_WEAPON, arrsCNPCSounds[SND_STOMP], VOL_NORM, ATTN_NORM, 0, PITCH_NORM + Math.RandomLong(-10, 10) );
		...
		if( tr.pHit !is null and tr.pHit !is pev.owner )
		{
			...
			if( pEntity !is null )
			{
				...
				if( bDealDamage and (pEntity.pev.flags & FL_CLIENT) == 0 or ((pEntity.pev.flags & FL_CLIENT) != 0 and CNPC::PVP) )
					pEntity.TakeDamage( pev, pevOwner, RANGE_DAMAGE, DMG_SONIC );
			}
		}
